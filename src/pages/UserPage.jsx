...
// pages/UserPage.jsx - Fixed version with proper store integration
...
const UserPage = () => {
  ...
  const getCurrentSection = () => {
    ...
    return ['favorites', 'watchlist', 'watched', 'profile'].includes(path) ? path : 'favorites';
  };
  ...
  const tabs = [
    { id: "favorites", label: "Favorites", icon: Heart, count: getTabCount("favorites") },
    { id: "watchlist", label: "Watchlists", icon: BookOpen, count: getTabCount("watchlist") },
    { id: "watched", label: "Watched", icon: Eye, count: getTabCount("watched") },
    ...
    if (filteredMovies.length === 0) {
      const emptyStateConfig = {
        ...
        watchlist: {
          ...
          description: "Create your first watchlist to organize movies you want to watch.",
        },
        watched: {
          icon: Eye,
          title: "No watched movies yet",
          description: "Movies you've watched will appear here.",
        },
      };
          ...
          <div key={movie.movieId || movie.id || movie._id}>
            {viewMode === "grid" ? (
              <MovieCard
                ...
                onMarkWatched={activeTab === 'watchlist' ? () => handleMarkAsWatched(movie) : null}
              />
            ) : (
              <div className="flex items-center space-x-4 p-4 bg-gray-800 rounded-lg hover:bg-gray-750 transition-colors">
                <img
                  src={movie.poster || movie.poster_path ? 
                    `https://image.tmdb.org/t/p/w200${movie.poster || movie.poster_path}` :
                ...
                <div className="flex-1">
                  <h3 
                    className="font-semibold hover:text-red-400 cursor-pointer transition-colors"
                    onClick={() => handleMovieClick(movie)}
                  >
                    {movie.title}
                  </h3>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    ...
                    {movie.watchedAt || movie.addedAt || movie.dateAdded ? 
                      new Date(movie.watchedAt || movie.addedAt || movie.dateAdded).toLocaleDateString() :
                      'Unknown date'
                    }
                  </p>
                </div>
        ...
        {Array.isArray(watchlists) && watchlists.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {watchlists.map((watchlist) => (
              <div key={watchlist._id || watchlist.id} className="bg-gray-800 rounded-lg p-4 hover:bg-gray-750 transition-colors">
                ...
                {watchlist.description && (
                  <p className="text-sm text-gray-400 mb-4">{watchlist.description}</p>
                )}
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    variant="ghost"
                    size="small"
                    onClick={() => navigate(`/watchlist/${watchlist._id || watchlist.id}`)}
                  >
                    View
                  </Button>
                </div>
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-gray-300">{userData.firstName || "Not specified"}</p>
              )}
            </div>
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-gray-300">{userData.lastName || "Not specified"}</p>
              )}
            </div>
            ...
            <div className="md:col-span-2">
              ...
              {isEditing ? (
                ...
                <p className="text-gray-300">{userData.bio || "No bio added yet."}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-gray-800 rounded-lg p-6 text-center hover:bg-gray-750 transition-colors">
            ...
            <h3 className="text-2xl font-bold">{favorites?.length || 0}</h3>
            <p className="text-gray-400">Favorites</p>
          </div>
          <div className="bg-gray-800 rounded-lg p-6 text-center hover:bg-gray-750 transition-colors">
            <BookOpen className="h-8 w-8 text-blue-500 mx-auto mb-2" />
            <h3 className="text-2xl font-bold">{watchlists?.length || 0}</h3>
            <p className="text-gray-400">Watchlists</p>
          </div>
          <div className="bg-gray-800 rounded-lg p-6 text-center hover:bg-gray-750 transition-colors">
            <Eye className="h-8 w-8 text-green-500 mx-auto mb-2" />
            <h3 className="text-2xl font-bold">{watchedMovies?.length || 0}</h3>
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          ...
          <div className="space-y-3">
            {[...(favorites || []), ...(watchedMovies || [])]
              ...
              .sort((a, b) => new Date(b.addedAt || b.watchedAt || 0) - new Date(a.addedAt || a.watchedAt || 0))
              .slice(0, 5)
              .map((movie) => (
                <div key={movie.movieId || movie.id} className="flex items-center space-x-3 p-2 hover:bg-gray-750 rounded-lg transition-colors">
                  <img
                    src={movie.poster || movie.poster_path ? 
                      `https://image.tmdb.org/t/p/w200${movie.poster || movie.poster_path}` :
                  ...
                  <div className="flex-1">
                    <p className="text-sm">
                      {movie.watchedAt ? 'Watched' : 'Added'} <span className="font-medium text-white cursor-pointer hover:text-red-400" onClick={() => handleMovieClick(movie)}>{movie.title}</span> {movie.watchedAt ? '' : 'to favorites'}
                    </p>
                    <p className="text-xs text-gray-400">
                      {new Date(movie.addedAt || movie.watchedAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
              ))}
            {(!favorites || favorites.length === 0) && (!watchedMovies || watchedMovies.length === 0) && (
          ...
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-3xl font-bold">
              {activeTab === "profile" ? "Profile" : 
               ...
               activeTab === "watchlist" ? "My Watchlists" :
